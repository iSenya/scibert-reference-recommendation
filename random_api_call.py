import json, requests, string, random, langdetect
...
num_random_papers = 100  # Number of random papers to retrieve
attributes = "paperId,title,abstract,references.title,references.abstract,references.paperId"

# Generate random paper IDs in the specified format
random_paper_ids = [''.join(random.choices(string.ascii_lowercase + string.digits, k=40)) for _ in range(num_random_papers)]
...
for paperId in random_paper_ids:
